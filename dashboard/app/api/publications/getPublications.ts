...
export default async function getPublications(
	...
	const filterCondition = conditions.length
		...
		: "";
	const statement = `SELECT * FROM publication_record ${filterCondition} ${sortOrder} LIMIT ${limit} OFFSET ${offset}`;
	const publications = (await db.execute(sql.raw(statement))) as any[];
	...
	if (filter && filter.profile_id) {
		const statement = `SELECT COUNT(*) AS count FROM publication_record ${filterCondition}`;
